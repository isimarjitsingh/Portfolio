...
async function initializeDatabase() {
    try {
        ...
        console.error('Please check your Oracle connection settings in .env file');
        process.exit(1);
    }
}
...
app.get('/api/dashboard/stats', async (req, res) => {
    try {
        const [students, totalFees, collectedFees, pendingFees] = await Promise.all([
            ...
            connection.execute(`SELECT NVL(SUM(total_fee), 0) as total FROM students WHERE status = 'Active'`),
            connection.execute(`SELECT NVL(SUM(paid_amount), 0) as total FROM students WHERE status = 'Active'`),
            connection.execute(`SELECT NVL(SUM(balance_amount), 0) as total FROM students WHERE status = 'Active'`)
        ]);
    ...
    try {
        const result = await connection.execute(
            `SELECT s.*, u.username, u.email, c.class_name, ay.year_name,
                    (SELECT NVL(SUM(paid_amount), 0) FROM fee_receipts WHERE student_id = s.student_id) as total_paid
             FROM students s
...
app.post('/api/students', async (req, res) => {
    try {
        ...
        // Validation
        if (!username || !password || !email || !fullName || !admissionNumber || !firstName || !lastName || !dateOfBirth || !gender || !classId || !academicYearId || !totalFee) {
        ...
        if (err.code === 'ORA-00001') {
            res.status(400).json({ error: 'Username or admission number already exists' });
        } else {
            res.status(500).json({ error: 'Server error' });
        }
    }
...
app.post('/api/fee-receipts', async (req, res) => {
    try {
        ...
        // Validation
        if (!studentId || !academicYearId || !paidAmount || !paymentDate || !paymentMethodId) {
            return res.status(400).json({ error: 'Missing required fields' });
        }
        ...
        // Insert payment breakdown if provided
    ...
    try {
        ...
        let query = `
            SELECT c.class_name, 
                   ...
                   NVL(SUM(s.total_fee), 0) as total_fees,
                   NVL(SUM(s.paid_amount), 0) as collected_amount,
                   NVL(SUM(s.balance_amount), 0) as pending_amount,
                   ROUND((NVL(SUM(s.paid_amount), 0) / NVL(SUM(s.total_fee), 1)) * 100, 2) as collection_percentage
            FROM students s
    ...
    try {
        ...
        const result = await connection.execute(
            `SELECT TO_CHAR(fr.payment_date, 'MM') as month,
                    ...
                    NVL(SUM(fr.paid_amount), 0) as total_collection
             FROM fee_receipts fr
             WHERE TO_CHAR(fr.payment_date, 'YYYY') = :year AND fr.status = 'Paid'
...
app.get('/', (req, res) => {
    res.sendFile(path.join(__dirname, 'public', 'fees-index.html'));
});
